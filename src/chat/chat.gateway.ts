...
export class ChatGateway {
  ...
  private async handleConnection(client: Socket) {
    ...
    // отправка глобального чата клиенту
    const globalMessages = await this.chatGlobalService.getMessages();
    client.emit(SocketChatListener.GETGLOBALIST, globalMessages);
  }
  ...
  private async createRoomIfBothOnline(
    ...
    // Получаем диалог и отправляем участникам кешированные данные
    ...
    if (messages) {
      ...
      // если кеша нет, то ищем в бд
      const findArr = await this.chatService.getDialogue(roomName);
      if (findArr) {
        await this.chatRedisService.createRoomDialogue(roomName, findArr);
        this.emitRoomMessages(
          roomName,
          SocketChatListener.GETCASHDIALOGUE,
          findArr,
        ...
        // если в бд нет, то отсылаем пустой массив
  ...
  private async startChat(
    ...
    console.log('Начало чата с пользователем:', data);
